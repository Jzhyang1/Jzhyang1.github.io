const PokemonConfig = {
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; max-height: 300px; overflow-y: auto;">
                        ${this.stats.caughtPokemon.length === 0 ? 
                            ...
                            this.stats.caughtPokemon.map(pokemon => `
                                <div class="pokedex-entry" style="
                                    padding: 10px;
                                    border-radius: 8px;
                                    background: ${pokemon.shiny ? '#FFD700' : '#f5f5f5'};
                                    display: flex;
                                    align-items: center;
                                    gap: 10px;
                                    cursor: pointer;
                                ">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <div style="font-size: 0.8em; color: #666;">${pokemon.rarity}</div>
                                        ${pokemon.shiny ? '<div style="color: #FFD700;">✨ Shiny</div>' : ''}
                                    </div>
                                </div>
                            `).join('')
                        }
    ...
    updateMenu(activeTab = undefined, shouldShow = undefined) {
        ...
        if (activeTab === undefined) {
            activeTab = this.menu.querySelector('button[style*="background: rgb(240, 240, 240)"]')?.textContent || 'Stats';
        }
    ...
    toggleMenu() {
        if (!this.menu) {
            ...
            const activeTab = this.menu.querySelector('button[style*="background: rgb(240, 240, 240)"]')?.textContent || 'Stats';
    ...
    updatePokedexContent() {
        ...
        contentContainer.innerHTML = filteredPokemon.length === 0 ? 
                ...
                " onmouseover="this.style.transform='translateY(-5px)';this.style.boxShadow='0 5px 15px rgba(0,0,0,0.1)'"
                  onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='0 2px 10px rgba(0,0,0,0.05)'">
                    ...
                    <div style="text-align: center;">
                        <div style="font-weight: bold; font-size: 1.2em; color: #333;">${pokemon.name}</div>
                        <div style="color: #666; margin: 5px 0;">${getRarityStars(pokemon.rarity)}</div>
                        <div style="font-size: 0.9em; color: #888;">Caught: ${formatDate(pokemon.dateCaught)}</div>
                        ${pokemon.shiny ? '<div style="color: #FFD700; margin-top: 5px;">✨ Shiny</div>' : ''}
                        ...
                        " onmouseover="this.style.background='#ff6666'"
                          ...
                          onclick="event.stopPropagation(); window.walker.menuManager.confirmTrade(${pokemon.id})">
                            Trade
                        </button>
                    </div>
                </div>
