...
async function getPokemonByName(name) {
    ...
    const rarity = data.is_legendary ? 'legendary' : data.is_mythical ? 'mythical' : encounter_rate <= 10 ? 'rare' : encounter_rate <= 20 ? 'uncommon' : 'common';
    //stats based on rarity; range based on habitats
    const stats = {
        health: {common: 100, uncommon: 150, rare: 200, legendary: 250, mythical: 250}[rarity],
        attack: {common: 10, uncommon: 15, rare: 20, legendary: 40, mythical: 40}[rarity],
        attackSpeed: {common: 1, uncommon: 1, rare: 0.8, legendary: 0.5, mythical: 0.5}[rarity],
        speed: {common: 10, uncommon: 15, rare: 11, legendary: 5, mythical: 5}[rarity],
        range: {other: 12, urban: 5, cave: 10, forest: 15, "waters-edge": 17, mountain: 20, "rough-terrain": 22, grassland: 25, sky: 35, sea: 40, "rare": 70}[data.habitat?.name || 'other']
    };
    const pokemon = {
        name: data.name,
        id: data.id,
...
class UIComponents {
    ...
    initializeDataManager() {
        ...
        const handler = (event) => {
            if (event.data.type === 'GAME_INITIALIZE') {
                // only show the menu if this is the first time the game is initialized
                if (event.data.first) {
                    // Initialize with the how-to-play page
                    this.activeTab = 'help';
                    this.toggleMenu();
                }
    ...
    handleMessage(event) {
        ...
        if (type === 'DATA_UPDATE') {
            // Preserve the Pokemon data structure while updating stats
            this.stats = {
                ...this.stats,  // Keep existing stats
                ...data.stats,  // Update with new stats
                caughtPokemon: data.stats.caughtPokemon.map(pokemon => ({
                    ...pokemon,  // Keep all Pokemon properties including evolution
                    evolution: pokemon.evolution  // Explicitly preserve evolution data
                }))
            };
    ...
    async toggleMenu() {
        if (this.menu.style.display === 'none') {
            // If we need storage access, request it on first interaction
...
class PokemonWalker {
    constructor(options = {}) {
        this.options = {
            ...
            spawnInterval: options.spawnInterval || 1500,
    ...
    async createPokemon(times = 1) {
        ...
        // Possibly generate more pokemon
        await this.createPokemon(times - 1);
    }
    ...
    catchPokemon(pokemon) {
        ...
        const caughtPokemon = {
            ...
            description: pokemonObj.description,
