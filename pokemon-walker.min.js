...
class UIComponents {
    ...
    initializeDataManager() {
        ...
        iframe.onload = async () => {
            ...
            if (document.hasStorageAccess) {
                ...
                if (!hasAccess) {
                    // We'll request access on first menu interaction
                    this.needsStorageAccess = true;
                }
            } else {
                console.warn('No hasStorageAccess function found');
            }
            this.requestData();
        };
    }
    ...
    async toggleMenu() {
        if (this.menu.style.display === 'none') {
            ...
            if (this.needsStorageAccess && document.requestStorageAccess) {
                try {
                    await document.requestStorageAccess({localStorage: true});
                    this.needsStorageAccess = false;
                    // Reload the iframe to get fresh storage access
                    this.dataManagerFrame.src = this.dataManagerFrame.src;
                } catch (e) {
                    console.warn('Storage access request failed:', e);
                }
            } else if (this.needsStorageAccess) {
                console.warn('Storage access request failed:', this.needsStorageAccess);
            }
